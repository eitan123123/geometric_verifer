...
def checkRequirementWithDetailedError(state: GeometricState, req: TheoremRequirement) -> tuple[
    ...
    if req.type == "parallelogram":
        ...
        points_str = ''.join(p.name for p in needed_points)
        ...
        if not is_valid_parallelogram_ordering(points_str):
            ...
            if found_fact:
                ...
                error = GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message=f"Invalid parallelogram ordering '{points_str}'. Must be a cyclic rotation. "
                            f"Valid orderings for this parallelogram are: {valid_orders}",
                    details="Premise violation: The theorem requires a valid parallelogram ordering",
                    related_objects=req.points
                )
                return False, error

        # Now look for matching parallelogram fact
        found_fact = None
        for fact in state.facts:
            if fact.type == "Parallelogram":
                if points_equal_with_rotations(fact.points, needed_points, "Parallelogram"):
                    found_fact = fact
                    break
        ...
        if not found_fact:
            error = GeometricError(
                ...
                details="Premise violation: The theorem requires these points to form a parallelogram",
                related_objects=req.points
            )
            return False, error
