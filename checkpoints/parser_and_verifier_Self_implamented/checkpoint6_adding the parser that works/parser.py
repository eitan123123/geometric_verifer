...
def create_initial_state(content: str) -> Tuple[GeometricState, Dict[str, Point]]:
    ...
    for line in construction.split('\n'):
        ...
        if line.startswith('Shape('):
            shape_str = line[6:-1]
            shape_points = create_points_from_shape(shape_str, points_dict)
            if shape_points:
                initial_facts.append(GeometricFact(type_="Shape", points=shape_points))
    ...
    for line in text_cdl.split('\n'):
        ...
        if line.startswith('Parallelogram('):
            ...
            # Extract points like CD,AB from ParallelBetweenLine(CD,AB)
            ...
            if len(pairs) == 2:
                # Clean any parentheses and whitespace from the pairs
            ...
            if len(pairs) == 2:
                ...
                # Create perpendicular angle (BCA) with measure 90°
...
if __name__ == "__main__":
    result = verify_geometric_proof("question3_correct")
