...
def propagateEqualAnglesHelper(state: GeometricState) -> Optional[GeometricState]:
    """Enhanced propagation that handles symbolic expressions"""
        ...
        if fact.type == "AngleEqual" and len(fact.angles) == 2:
            ...
            if measure1 and measure2:
                try:
                    ...
                    print(f"Warning: Error in angle equality: {e}")
            elif measure1 and not measure2:
                new_angles.append(AngleStatement(angle=angle2,
                                               measure=measure1.measure))
                changed_something = True
            elif measure2 and not measure1:
                new_angles.append(AngleStatement(angle=angle1,
                                               measure=measure2.measure))
                changed_something = True

    if changed_something:
        try:
            # Try to solve equations after propagation
            new_state = GeometricState(facts=state.facts,
                                     lengths=state.lengths,
                                     angles=new_angles)
            new_state.solve_equations()
            return new_state
        except Exception as e:
            print(f"Warning: Error in solving equations: {e}")
            return None
    else:
        return None
...
def applyConclusion(state: GeometricState, concl: TheoremConclusion) -> GeometricState:
    ...
    if concl.type == "angle_measure":
        if len(concl.angles) == 1:
            ...
            # In your applyConclusion function, modify the "complementary_angles" case:
            elif "complementary_angles" in concl.relatedFacts:
                if len(concl.angles) == 1:
                    targetAngle = concl.angles[0]
                    # Find the known angle with same vertex
                    known = next((x for x in new_angles if x.angle != targetAngle
                                  and x.angle.vertex == targetAngle.vertex), None)
                    if known:
                        # Calculate complementary angle value (180° - known angle)
    ...
    # angle_sum
        ...
        if len(concl.angles) == 3:
            ...
            if m1 and m2:
                # Use the sympy expressions for subtraction
