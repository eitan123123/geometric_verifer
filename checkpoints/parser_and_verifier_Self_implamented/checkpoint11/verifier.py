...
class GeometricFact:
    def __init__(
        ...
        if points:
            if self.type == "Collinear":
                ...
                # For parallelograms, validate ordering and normalize
                points_str = ''.join(p.name for p in points)
                if not is_valid_parallelogram_ordering(points_str):
                    # Convert to valid ordering (ABCD format)
                    point_names = sorted(''.join(p.name for p in points))
                    points = [next(p for p in points if p.name == name) for name in point_names]
                # Normalize to start with smallest point name
                min_idx = min(range(len(points)), key=lambda i: points[i].name)
                self.points = points[min_idx:] + points[:min_idx]
            elif self.type in ["Triangle", "Shape"]:
    ...
    def __eq__(self, other):
        ...
        if self.type == "Collinear":
            ...
            # Consider cyclic rotations for shapes and parallelograms
            return points_equal_with_rotations(self.points, other.points, self.type)
...
parallelogramOppositeAngleTheorem = TheoremRule(
    ...
    requirements=[
        TheoremRequirement(type_="parallelogram", points=[])  # lowercase to match the validation
    ],
    conclusions=[
...
def get_opposite_angles_in_parallelogram(points: List[Point]) -> List[tuple[Angle, Angle]]:
    """
    Given points of a parallelogram, returns pairs of opposite angles.
    """
    if len(points) != 4:
        return []

    # First pair: DAB vs BCD
    angle1 = makeAngle(points[3], points[0], points[1])  # DAB
    angle2 = makeAngle(points[1], points[2], points[3])  # BCD

    # Second pair: ABC vs CDA
    angle3 = makeAngle(points[0], points[1], points[2])  # ABC
    angle4 = makeAngle(points[2], points[3], points[0])  # CDA
...
def points_equal_with_rotations(points1: List[Point], points2: List[Point], fact_type: str = None) -> bool:
    """
    ...
    - Parallelograms: consider only cyclic rotations (ABCD = BCDA = CDAB = DABC)
    """
    ...
    elif fact_type == "Parallelogram" or fact_type == "Shape":  # Consider cyclic rotations only
        rotations = get_cyclic_rotations(points1)
        return points2 in rotations
    else:  # Default case
...
def is_valid_parallelogram_ordering(points_str: str, base_str: Optional[str] = None) -> bool:
    """
    Check if a parallelogram ordering is valid.
    - If base_str provided, points_str must be cyclic rotation of it
    - If no base_str, points must be in cyclic order (ABCD, BCDA, CDAB, DABC)
    Invalid orders are ABDC, ACDB etc which don't maintain cyclic ordering
    """
    if len(points_str) != 4:
        return False
    ...
    if base_str:
        ...
        # For first parallelogram definition, check if points form valid cycle
        # Take first point and see if we can find a valid cycle
        p1 = points_str[0]
        p2 = points_str[1]
        p3 = points_str[2]
        p4 = points_str[3]

        # In a valid parallelogram, opposite sides are parallel
        # So points must maintain cyclic order: p1->p2->p3->p4->p1
        # Check if it's a valid cycle by verifying each point appears exactly once
        # and they maintain cyclic order
        if len(set([p1, p2, p3, p4])) != 4:
            return False

        # Valid cycles are p1->p2->p3->p4 or p1->p4->p3->p2
        # ABCD, BCDA, CDAB, DABC are valid
        # ABDC, ACDB etc. are invalid
        valid_cycles = [
            points_str[0] + points_str[1] + points_str[2] + points_str[3],
            points_str[1] + points_str[2] + points_str[3] + points_str[0],
            points_str[2] + points_str[3] + points_str[0] + points_str[1],
            points_str[3] + points_str[0] + points_str[1] + points_str[2]
        ]
        return points_str in valid_cycles
...
def checkRequirementWithDetailedError(state: GeometricState, req: TheoremRequirement) -> tuple[
    ...
    if req.type == "parallelogram":
        needed_points = req.points
        found_fact = None

        # First, find the parallelogram fact in the state
        para_fact = next((f for f in state.facts if f.type == "Parallelogram"), None)
        if not para_fact:
            return False, GeometricError(
                tier=ErrorTier.TIER2_PREMISE,
                message="No parallelogram found in state"
            )

        # Get the state's parallelogram point sequence
        state_points_str = ''.join(p.name for p in para_fact.points)
        # Get the premise point sequence
        premise_points_str = ''.join(p.name for p in needed_points)

        # Check if premise points form a valid cyclic rotation of state points
        valid_rotations = [state_points_str[i:] + state_points_str[:i] for i in range(4)]
        if premise_points_str not in valid_rotations:
            # Give helpful error message showing valid rotations
            return False, GeometricError(
                tier=ErrorTier.TIER2_PREMISE,
                message=f"Invalid parallelogram point order: {premise_points_str}",
                details=f"Points must be in cyclic order from state's parallelogram. Valid orders are: {valid_rotations}",
                related_objects=needed_points
            )
