...
class GeometricTheorem:
    ...
    def validate_theorem_premises(self, theorem_name: str, args: List[str], premise: str) -> Tuple[
        ...
        if theorem_name == "adjacent_complementary_angle":
            ...
            # First check if the theorem is called with a valid parallelogram name
            ...
            # Check if this parallelogram exists in our TEXT_CDL facts

            if theorem_para not in self.parallelograms:
                return False, GeometricError(
                    ...
                    details=f"Available parallelograms: {', '.join(self.parallelograms) if self.parallelograms else 'none'}"

                )

            if len(theorem_para) != 4:
                return False, GeometricError(

                    tier=ErrorTier.TIER2_PREMISE,

                    message=f"Parallelogram must have exactly 4 points, got: {theorem_para}"

                )

            # Create the parallel pairs we're looking for

            sides = []
            # Add all possible orientations of each pair
            pair1 = (theorem_para[0] + theorem_para[1], theorem_para[2] + theorem_para[3])  # AB,CD
            pair2 = (theorem_para[1] + theorem_para[2], theorem_para[3] + theorem_para[0])  # BC,DA

            # For each pair, add all possible orientations
            def get_all_orientations(line1, line2):
                # Original order
                yield (line1, line2)
                yield (line2, line1)
                # Reversed order
                ...
                yield (line2, line1[::-1])
                yield (line2[::-1], line1)
                yield (line1[::-1], line2[::-1])
                yield (line2[::-1], line1[::-1])

            # Generate all possible variations of the parallel pairs
            all_variations = set()
            for p1, p2 in [pair1, pair2]:
                for variation in get_all_orientations(p1, p2):
                    all_variations.add(variation)

            print(f"Checking for any of these parallel pair variations: {all_variations}")
            print(f"Available parallel pairs: {self.parallel_pairs}")

            # Check if any variation exists in our parallel pairs
            valid_pair1 = any(var in self.parallel_pairs for var in get_all_orientations(*pair1))
            valid_pair2 = any(var in self.parallel_pairs for var in get_all_orientations(*pair2))

            if not (valid_pair1 and valid_pair2):
                return False, GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message=f"Missing parallel sides for parallelogram {theorem_para}",
                    details=f"Required parallel pairs: {theorem_para[0]}{theorem_para[1]}||{theorem_para[2]}{theorem_para[3]} and {theorem_para[1]}{theorem_para[2]}||{theorem_para[3]}{theorem_para[0]}"
                )
            ...
            if 'TEXT_CDL' in sections:
                for line in sections['TEXT_CDL']:
                    if line.startswith('Collinear('):
                        ...
                        self.parallelograms.add(para_name)
