...
class GeometricTheorem:
    ...
    def verify_goal_length(self, p1: str, p2: str, expected: float, epsilon: float = 1e-8) -> tuple:
        ...
        if self.solver.check() == sat:
            ...
            if temp_solver1.check() != sat:
                ...
                model = self.solver.model()
                computed_value = float(model.eval(length_var).as_decimal(10).rstrip('?'))

                error_msg = f"Failed to prove length goal: proof determines {normalized} = {computed_value}, not {expected}."
                length_report += f"Error: Your answer was {expected}, but the proof uniquely determines length {normalized} to be {computed_value}\n"

                # Generate a detailed report using the new line-specific function
                detailed_report = self.generate_length_analysis_report(goal_line, expected, computed_value,
                                                                       "incompatible")
                length_report += f"\n{detailed_report}\n"

                print(f"Error: Proof determines length {normalized} = {computed_value}, not {expected}")
                error = GeometricError(
                    tier=ErrorTier.TIER3_GOAL_NOT_REACHED,
                    message=error_msg,
                    details=f"Goal was: LengthOfLine({normalized}) = {expected} but proof determines {computed_value}"
                )
                print(f"\nError in {error.tier.name}: {error.message}")
    ...
    def generate_goal_analysis_report(self, goal_angle, expected, alt_value, solver_state="multiple_values"):
        ...
        if solver_state == "unsatisfiable":
            ...
            report += f"The constraints in your proof uniquely determine angle {goal_angle} to be {alt_value}°, not {expected}°.\n"
            report += "This means your proof leads to a different value than your submitted answer.\n"
        else:  # multiple_values
    ...
    def verify_algebraic_goal(self, goal_angle: str, expected: float, epsilon: float = 1e-8) -> tuple:
        ...
        angle_report += f"Your answer: {expected}°\n\n"
        ...
        if self.solver.check() == sat:
            ...
            if temp_solver1.check() != sat:
                # Get the actual computed value from the model
                model = self.solver.model()
                computed_value = float(model.eval(goal_var).as_decimal(10).rstrip('?'))

                error_msg = f"Failed to prove angle goal: proof determines {goal_angle} = {computed_value}°, not {expected}°."
                angle_report += f"Error: Your answer was {expected}°, but the proof uniquely determines angle {goal_angle} to be {computed_value}°\n"
                angle_report += "The geometric constraints lead to a different value than your submitted answer.\n"

                # Generate a detailed report for this case
                detailed_report = self.generate_goal_analysis_report(goal_angle, expected, computed_value,
                                                                     "incompatible")
                angle_report += f"\n{detailed_report}\n"

                print(f"Error: Proof determines angle {goal_angle} = {computed_value}°, not {expected}°")
                error = GeometricError(
                    tier=ErrorTier.TIER3_GOAL_NOT_REACHED,
                    message=error_msg,
                    details=f"Goal was: MeasureOfAngle({goal_angle}) = {expected}° but proof determines {computed_value}°"
                )
                print(f"\nError in {error.tier.name}: {error.message}")
                if error.details:
                    print("Details:", error.details)
...
if __name__ == "__main__":
    result, feedback = verify_geometric_proof(
        "/Users/eitan/Desktop/lean/lean_python/questions/the new format for questions after jan_17/new_45_questions/question_1975/question1975_gt",print_output=False)
