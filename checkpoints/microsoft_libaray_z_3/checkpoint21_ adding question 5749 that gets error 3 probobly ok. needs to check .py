    ...
    def verify_goal_length(self, p1: str, p2: str, expected: float, epsilon: float = 1e-10) -> bool:
        ...
        if length_var is not None:
            ...
            if verify_solver.check() == sat:
                ...
                # Also handle any other special strings like "oo" or "-oo" if you want:
                if val_str in ("oo", "+oo"):
                    val_str = "inf"
                elif val_str == "-oo":
                    val_str = "-inf"
                ...
                ### NEW: Check if there's another distinct solution
                # We'll copy the same constraints into a new solver
                verify_solver_alt = Solver()

                # Re-add the same constraints (non-negatives, known lengths, proven relationships)
                for var in self.lengths.values():
                    verify_solver_alt.add(var > 0)

                for line, value in self.known_length_values.items():
                    var = self.lengths.get(f"length_{line}")
                    if var is not None:
                        verify_solver_alt.add(var == value)

                for rel_type, *parts in self.proven_length_relationships:
                    if rel_type == "sum":
                        a1, a2, total = parts
                        a1_var = self.add_length(a1[0], a1[1])
                        a2_var = self.add_length(a2[0], a2[1])
                        t_var = self.add_length(total[0], total[1])
                        verify_solver_alt.add(t_var == a1_var + a2_var)
                    elif rel_type == "pythagorean":
                        l1, l2, h = parts
                        l1_var = self.add_length(l1[0], l1[1])
                        l2_var = self.add_length(l2[0], l2[1])
                        h_var = self.add_length(h[0], h[1])
                        verify_solver_alt.add(l1_var * l1_var + l2_var * l2_var == h_var * h_var)
                        verify_solver_alt.add(l1_var + l2_var > h_var)
                        verify_solver_alt.add(h_var > l1_var)
                        verify_solver_alt.add(h_var > l2_var)
                    elif rel_type == "ratio":
                        side1, side2, tri1, tri2 = parts
                        ratio_key = self.normalize_similar_triangles(tri1, tri2)
                        if ratio_key in self.triangle_ratios:
                            old_ratio_var = self.triangle_ratios[ratio_key]
                            new_ratio_var = Real(f"verify_ratio_{ratio_key[0]}_{ratio_key[1]}")
                            verify_solver_alt.add(new_ratio_var >= 0)

                            s1_var = self.add_length(side1[0], side1[1])
                            s2_var = self.add_length(side2[0], side2[1])
                            verify_solver_alt.add(s1_var == s2_var * new_ratio_var)

                # Now add: "the length_var must NOT equal the previously found numeric solution"
                verify_solver_alt.add(length_var != calculated_value)

                # Check if it is still satisfiable
                if verify_solver_alt.check() == sat:
                    # That means there's a second distinct solution
                    print("Multiple solutions are possible for", normalized)
                else:
                    print("The solution for", normalized, "is unique")
                ### END-OF-NEW
                ...
                if length_match:
                    ...
                    if 'ANSWER' in sections and sections['ANSWER']:
                        ...
                        try:
                            ...
                            # if that fails, parse as a fraction
    ...
    Optional[GeometricError]:
        ...
        if theorem_name == "parallelogram_property_opposite_angle_equal":
            ...
            if match:
                ...
                # Record it in proven relationships (if you want to keep track):
                self.proven_angle_relationships.append(
                    ("sum", [a1, a2, a3], "180_for_triangle")
                )
            else:
                return GeometricError(
                    tier=ErrorTier.TIER1_THEOREM_CALL,
...
if __name__ == "__main__":
    result = verify_geometric_proof(
        "/questions/the new format for questions after jan_17/new_7_questions/question_5749/question5749_v1")
