...
class GeometricTheorem:
    ...
    def validate_theorem_premises(self, theorem_name: str, args: List[str], premise: str) -> Tuple[
        ...
        if theorem_name == "adjacent_complementary_angle":
            ...
            # Expected theorem call: parallel_property_collinear_extend(1, GA, HD, J)
            # Expected premise (for example): "ParallelBetweenLine(GA,HD)&Collinear(JGA)"
            token1 = args[1]  # e.g., "GA"
            token2 = args[2]  # e.g., "HD"
            token3 = args[3]  # e.g., "J"
            # Check that the premise contains ParallelBetweenLine(token1, token2)
            if not re.search(
            ...
            # Check that the premise contains a Collinear fact involving token3 and token1.
            # For example, if token1 is "GA", we expect a fact like "Collinear(JGA)"
            if not re.search(r'Collinear\(\s*' + re.escape(token3 + token1) + r'\s*\)', premise):
                return return_error(GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message=f"Premise must include Collinear({token3}{token1}).",
                    details=f"Premise provided: {premise}"
                ))
            return True, None
            ...
            if 'TEXT_CDL' in sections:
                for line in sections['TEXT_CDL']:
                    if line.startswith('Equal(MeasureOfAngle('):
                        ...
                        # New branch to handle arc measures generically.
            ...
            if 'GOAL_CDL' in sections:
                ...
                if arc_length_match:
                    ...
                    if 'ANSWER' in sections and sections['ANSWER']:
                        ...
                        # Use your normalization and naming convention for arc length variables.
                ...
                if general_match:
                    ...
                    if self.solver.check() == sat:
                        ...
                        if answer_str is None:
                            ...
                            return False
                        # Special case for a Sub expression
                                ...
                                if m1 and m2:
                                    ...
                                    if circle in self.circle_areas and tri in self.triangle_areas:
                                        ...
                                        try:
                                            ...
                                            print("Error converting area_circle to float using Fraction:", e)
                                            return False
                                        try:
                                            area_triangle_val = float(Fraction(str(area_triangle).replace('?', '')))
                                        except Exception as e:
                                            print("Error converting area_triangle to float using Fraction:", e)
                                            return False
                                        ...
                                        if abs(computed_value - model_answer) < epsilon:
                                            ...
                                            if error.details:
                                                print("Details:", error.details)
                                            return False
                        # Fallback: evaluate the goal expression generally.
                        mapping = {}
    ...
    def adding_conclution(self, theorem_name: str, args: List[str], premise: str, conclusions: List[str]) -> \
        ...
        if theorem_name == "parallelogram_property_opposite_angle_equal":
            ...
            # Expected conclusion list:
            # ["ParallelBetweenLine(JG,HD)", "ParallelBetweenLine(JA,HD)"]
            token1 = args[1]  # e.g., "GA"
            token2 = args[2]  # e.g., "HD"
            token3 = args[3]  # e.g., "J"
            # We interpret token1 as having two letters (e.g., "G" and "A").
            # Then we “extend” the parallel property by replacing token1’s letters with token3.
            new_line1 = token3 + token1[0]  # e.g., if token1 = "GA", then new_line1 becomes "JG"
            new_line2 = token3 + token1[1]  # e.g., new_line2 becomes "JA"
            # Now add the facts that these new lines are parallel to token2.
            # (Depending on your system, you might add these as constraints or simply record them.)
            self.parallel_pairs.add((new_line1, token2))
            self.parallel_pairs.add((new_line2, token2))
            print(
                f"Added parallel extension: ParallelBetweenLine({new_line1},{token2}) and ParallelBetweenLine({new_line2},{token2})")
...
if __name__ == "__main__":
    result = verify_geometric_proof(
        "/Users/eitan/Desktop/lean/lean_python/questions/the new format for questions after jan_17/new_45_questions/question_127/question127_gt")
