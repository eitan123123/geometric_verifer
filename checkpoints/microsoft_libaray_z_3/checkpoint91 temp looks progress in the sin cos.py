...
class GeometricTheorem:
    ...
    def evaluate_trig_function(self, func_name, angle_token, model_answer):
        ...
        if self.solver.check() == sat:
            ...
            # Third approach: Try to derive from triangle relationships
            # For this, we need to find the triangle containing this angle
            containing_triangles = []
            for polygon in self.polygons:
                if len(polygon) == 3 and angle_token[1] in polygon:
                    # Verify the angle belongs to this triangle
                    if angle_token[0] in polygon and angle_token[2] in polygon:
                        containing_triangles.append(polygon)

            for triangle in containing_triangles:
                try:
                    # For a right triangle, we can derive the angle from the sides
                    if triangle in self.right_triangles:
                        ...
                        for i in range(3):
                            ...
                            v2 = triangle[(i + 1) % 3]
                            v3 = triangle[(i + 2) % 3]
                            angle_name = self.normalize_angle_name(v1 + v2 + v3)
                            angle_temp = self.add_angle(angle_name[0], angle_name[1], angle_name[2])

                            # Check if this is the right angle
                            right_angle_temp = Solver()
                            for c in self.solver.assertions():
                                right_angle_temp.add(c)
                            right_angle_temp.add(angle_temp != 90)

                            if right_angle_temp.check() == unsat:
                                # This is the right angle, so the opposite sides form the hypotenuse
                                right_angle_vertex = v2
                                other_vertices = [v1, v3]

                                # If our target angle is at this vertex, can't use right angle rules
                                if right_angle_vertex == angle_token[1]:
                                    continue

                                hypotenuse = other_vertices[0] + other_vertices[1]

                                # Find which of the other vertices is in our target angle
                                if angle_token[0] == right_angle_vertex:
                                    adjacent = right_angle_vertex + angle_token[2]
                                    opposite = right_angle_vertex + angle_token[0]
                                elif angle_token[2] == right_angle_vertex:
                                    adjacent = right_angle_vertex + angle_token[0]
                                    opposite = right_angle_vertex + angle_token[2]
                                else:
                                    # Can't determine which is which
                                    continue

                                break

                        # If we found the needed sides, calculate the trig functions
                        if hypotenuse and adjacent and opposite:
                            ...
                            print(f"Calculated {func_name}({angle_token}) = {calculated_val} from triangle sides")
                    ...
                    print(f"Error deriving trig value from triangle: {e}")

        # If we got here, we couldn't determine a unique value
        return False, None, "multiple_values" if self.solver.check() == sat else "unsatisfiable"
                        ...
                        try:
                            ...
                            if side2_val > 0 and side3_val > 0:  # Avoid division by zero
                                ...
                                if -1 <= expected_cos <= 1:  # Valid cosine value
                                    ...
                                    # Add constraint to set cos variable to this value

                                    self.solver.add(cos_var == expected_cos)

                                    print(f"Set cos({angle_str}) = {expected_cos}")

                                    # Calculate the corresponding sine value

                                    import math

                                    expected_sin = math.sqrt(1 - expected_cos ** 2)

                                    # Create sine variable if needed
                                    ...
                                    self.solver.add(self.variables[sin_var_name] == expected_sin)

                                    print(f"Set sin({angle_str}) = {expected_sin}")

                                    # Also try to set the angle value
