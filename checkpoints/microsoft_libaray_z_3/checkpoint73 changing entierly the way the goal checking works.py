...
class GeometricTheorem:
    ...
    def generate_detailed_feedback(self, goal_type, goal_token, expected_value, computed_value=None,
        ...
        """Generate focused and targeted feedback report for a goal verification result."""
        # Initialize the report with a standardized header
        report = f"Analysis Report for {self.question_name}\n"
        report += "=" * 60 + "\n\n"

        # Add goal information based on type
        if goal_type == "angle":
            report += f"You tried to find the measure of angle {goal_token}\n"
        elif goal_type == "length":
            report += f"You tried to find the length of line {goal_token}\n"
        elif goal_type == "arc_measure":
            report += f"You tried to find the measure of arc {goal_token}\n"
        elif goal_type == "arc_length":
            report += f"You tried to find the length of arc {goal_token}\n"
        elif goal_type == "cosine":
            report += f"You tried to find the cosine of angle {goal_token}\n"
        elif goal_type == "sine":
            report += f"You tried to find the sine of angle {goal_token}\n"
        elif goal_type == "sum":
            tokens = goal_token.split('+')
            report += f"You tried to find the sum of lines {tokens[0]} + {tokens[1]}\n"
        elif goal_type == "ratio":
            tokens = goal_token.split('/')
            report += f"You tried to find the ratio of lines {tokens[0]} / {tokens[1]}\n"
        elif goal_type == "perimeter":
            report += f"You tried to find the perimeter of triangle {goal_token}\n"
        elif goal_type == "quad_area":
            report += f"You tried to find the area of quadrilateral {goal_token}\n"
        elif goal_type == "general":
            report += f"You tried to find the value of {goal_token}\n"

        report += f"Your answer: {expected_value}\n\n"

        # Extract points from goal_token based on goal type
        goal_points = list(goal_token)

        # Gather and present focused facts directly related to the goal
        report += f"In the premises you've had:\n"
        report += "-" * 60 + "\n"

        # Collect related facts that contain ALL goal points
        ...
        if related_facts:
            ...
            report += f"No direct facts involving all points in {goal_token} were found in the premises.\n\n"

        # Add theorems related to this goal
        report += f"These are the theorems that have to do with goal {goal_token} in your proof:\n"
        report += "-" * 60 + "\n"
        ...
        for theorem_info in self.theorem_sequence:
            ...
            # For angles, arcs, etc., check normalized forms
            if not is_related and goal_type in ["angle", "arc_measure", "arc_length"]:
                for conclusion in theorem_info["conclusions"]:
                    # Check for relevant patterns like MeasureOfAngle(ABC), MeasureOfArc(ABC)
        ...
        if related_theorems:
            ...
            report += f"None. Your proof doesn't include any theorems that constrain {goal_token}.\n\n"
        ...
        if relevant_constraints:
            ...
            report += f"No direct constraints found involving {goal_token}.\n\n"

        # Add diagnosis based on status
        report += "Diagnosis:\n"
        report += "-" * 60 + "\n"
        ...
        if status == "unsatisfiable":
            ...
            report += "1. Incorrect values in premises\n"
            report += "2. Improper theorem application\n"
            report += "3. Conclusions that contradict earlier assertions\n"

        elif status == "incompatible":
            report += f"The geometric constraints in your proof uniquely determine the {goal_type} of {goal_token} to be {computed_value}, not Your answer:{expected_value}.\n"
            ...
            if goal_type == "arc_length":
                ...
                report += "Check that:\n"
                report += "1. Your angle measure values are correctly specified\n"
                report += "2. You've applied the angle theorems correctly\n"
                report += "3. Your calculations are accurate\n"

        elif status == "multiple_values":
            report += f"Your proof doesn't uniquely determine the {goal_type} of {goal_token}.\n"
            report += "Multiple solutions are possible with the current constraints.\n"
            if computed_value is not None:
                report += f"It could be Your answer: {expected_value} but also {computed_value}\n\n"
    ...
    def collect_related_facts(self, goal_points, goal_type=None):
        ...
        # 2. Angles that contain ALL goal points (in any order)
        related_angles = []
        ...
        for angle_name, angle_var in self.angles.items():
            ...
            # Check if angle contains ALL goal points and exactly matches in size
            if goal_points_set.issubset(angle_points_set) and len(angle_points_set) == len(goal_points_set):
        ...
        # 3. Arcs that contain ALL goal points (in any order)
        related_arcs = []
        for arc_name in self.arcs:
            ...
            # Check if arc contains ALL goal points
            if goal_points_set.issubset(arc_points_set):
                related_arcs.append(f"Arc {arc_points}")
        if related_arcs:
            related_facts["Arcs"] = related_arcs

        # 4. Circles where the goal points are part of key relationships
        related_circles = []
        for circle, center in self.circle_centers.items():
            # Only include if ALL goal points are involved in a meaningful way
            if circle in goal_points_set and set([circle, center]).issubset(goal_points_set):
                related_circles.append(f"Circle {circle} with center {center}")
        if related_circles:
            related_facts["Circles"] = related_circles

        # 5. Cocircular facts that contain ALL goal points
        related_cocircular = []
        seen_cocircular = set()  # To avoid duplicates
        for fact in self.cocircular_facts:
            fact_set = set(fact)
            if goal_points_set.issubset(fact_set):
        ...
        if related_cocircular:
            # Only show up to 3 cocircular facts to avoid overwhelming the user
            related_facts["Cocircular Points"] = related_cocircular[:3]
            if len(related_cocircular) > 3:
                related_facts["Cocircular Points"].append(f"...and {len(related_cocircular) - 3} more")

        # 6. Collinear facts that contain ALL goal points
        related_collinear = []
        for collinear in self.collinear_facts:
            collinear_set = set(collinear)
            if goal_points_set.issubset(collinear_set):
...
if __name__ == "__main__":
    result, feedback = verify_geometric_proof(
        "/Users/eitan/Desktop/lean/lean_python/questions/the new format for questions after jan_17/new_45_questions/question_464/question464_gt",print_output=False)
