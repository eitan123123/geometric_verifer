...
class GeometricTheorem:
    ...
    def validate_theorem_premises(self, theorem_name: str, args: List[str], premise: str) -> Tuple[
        ...
        if theorem_name == "adjacent_complementary_angle":
            ...
            if "Parallelogram" not in premise:
                return False, GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message="Missing parallelogram in premise",
                    details="parallelogram_property_opposite_angle_equal theorem requires parallelogram"
                )

            para_match = re.search(r'Parallelogram\((\w+)\)', premise)
            if not para_match:
                return False, GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
            ...
            points = para_match.group(1)
            if len(points) != 4:
                return False, GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message=f"Parallelogram must have exactly 4 points, got: {points}"
                )

            # Check if opposite sides are parallel
            if not ((points[0] + points[1], points[2] + points[3]) in self.parallel_pairs or
                    (points[2] + points[3], points[0] + points[1]) in self.parallel_pairs):
                return False, GeometricError(
                    tier=ErrorTier.TIER2_PREMISE,
                    message=f"Missing parallel sides for parallelogram {points}",
                    details=f"Required parallel pairs: {points[0]}{points[1]}||{points[2]}{points[3]} and {points[1]}{points[2]}||{points[3]}{points[0]}"
                )
    ...
    def parse_and_verify_proof(self, content: str) -> bool:
        ...
        try:
            ...
            # Initialize collinear facts list
            self.collinear_facts = []

            # First, add all points from CONSTRUCTION_CDL_EXTENDED and collect CONSTRUCTION_CDL facts
...
if __name__ == "__main__":
    theorem = GeometricTheorem()
    result = verify_geometric_proof("/Users/eitan/Desktop/lean/lean_python/questions/question2/question2_correct")
