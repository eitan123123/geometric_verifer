...
class GeometricTheorem:
    ...
    def verify_goal_length(self, p1: str, p2: str, expected: float, epsilon: float = 1e-10) -> bool:
        ...
        if length_var is not None:
            ...
            # Handle shared sides between right triangles
            for i, (type1, *parts1) in enumerate(self.proven_length_relationships):
                if type1 == "pythagorean":
                    for type2, *parts2 in list(self.proven_length_relationships)[i + 1:]:
                        if type2 == "pythagorean":
                            # Check for shared sides
                            for side1 in parts1:
                                for side2 in parts2:
                                    if self.normalize_line_name(side1) == self.normalize_line_name(side2):
                                        var1 = self.lengths[f"length_{self.normalize_line_name(side1)}"]
                                        var2 = self.lengths[f"length_{self.normalize_line_name(side2)}"]
                                        verify_solver.add(var1 == var2)

            # Check if solution exists
            if verify_solver.check() == sat:
                model = verify_solver.model()
                if length_var in model:
                    calculated_value = float(model[length_var].as_decimal(10))
...
if __name__ == "__main__":
    result = verify_geometric_proof(
        "/questions/the new format for questions after jan_17/new_7_questions/question_729_problem_in_the_order_of_simillarity_in_normal_gt/question729_v1")
