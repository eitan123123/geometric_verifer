...
class GeometricTheorem:
    ...
    def evaluate_trig_function(self, func_name, angle_token, model_answer):
        ...
        if self.solver.check() == sat:
            ...
            # Third approach: Find the right triangle and compute trigonometric values directly
            vertex = angle_token[1]  # Middle letter is the vertex

            # Find all triangles containing this angle
            containing_triangles = []
            for polygon in self.polygons:
                if len(polygon) == 3 and all(p in polygon for p in [angle_token[0], angle_token[1], angle_token[2]]):
                    containing_triangles.append(polygon)

            print(f"Found triangles containing angle {angle_token}: {containing_triangles}")

            # For each triangle, check if it's a right triangle and compute trig values
            for triangle in containing_triangles:
                # Check if this is a right triangle
                if self.normalize_triangle(triangle) in self.right_triangles:
                    print(f"Triangle {triangle} is a right triangle")

                    # Find the right angle vertex
                    right_angle_vertex = None
                    for t_vertex in triangle:
                        # Create angles with this vertex in the middle
                        other_vertices = [v for v in triangle if v != t_vertex]

                        angle_name = other_vertices[0] + t_vertex + other_vertices[1]
                        normalized_angle = self.normalize_angle_name(angle_name)
                        angle_key = f"angle_{normalized_angle}"

                        if angle_key in self.angles:
                            angle_test_var = self.angles[angle_key]

                            # Check if this is a right angle
                            temp_solver = Solver()
                            for c in self.solver.assertions():
                                temp_solver.add(c)
                            temp_solver.add(angle_test_var != 90)

                            if temp_solver.check() == unsat:
                                right_angle_vertex = t_vertex
                                print(f"Found right angle at vertex {t_vertex} in triangle {triangle}")
                                break

                    # If we found a right angle and it's not at our angle's vertex
                    if right_angle_vertex and right_angle_vertex != vertex:
                        # For a right triangle, use the direct trigonometric relationships

                        # The side opposite to the right angle is the hypotenuse
                        hypotenuse_vertices = [v for v in triangle if v != right_angle_vertex]
                        hypotenuse = ''.join(sorted(hypotenuse_vertices))

                        # Get our angle's other points (the arms of the angle)
                        arm1, arm2 = angle_token[0], angle_token[2]

                        # For a right triangle with right angle at C, and evaluating angle A:
                        # sin(A) = opposite/hypotenuse = BC/AB
                        # cos(A) = adjacent/hypotenuse = AC/AB

                        # Identify the sides relative to our angle
                        if vertex in hypotenuse_vertices:
                            # Our angle is at one end of the hypotenuse

                            # The side from our vertex to the right angle vertex is adjacent
                            adjacent = "".join(sorted([vertex, right_angle_vertex]))

                            # The other side is opposite
                            other_vertex = next(v for v in triangle if v != vertex and v != right_angle_vertex)
                            opposite = "".join(sorted([right_angle_vertex, other_vertex]))

                            # Get the lengths
                            hyp_var = self.add_length(hypotenuse_vertices[0], hypotenuse_vertices[1])
                            adj_var = self.add_length(adjacent[0], adjacent[1])
                            opp_var = self.add_length(opposite[0], opposite[1])

                            hyp_val = float(model.eval(hyp_var).as_decimal(10).rstrip('?'))
                            adj_val = float(model.eval(adj_var).as_decimal(10).rstrip('?'))
                            opp_val = float(model.eval(opp_var).as_decimal(10).rstrip('?'))

                            # Compute the trig value based on function name
                            if func_name == "sin":
                                calculated_val = opp_val / hyp_val
                            else:  # cos
                                calculated_val = adj_val / hyp_val

                            print(f"For angle {angle_token} in right triangle {triangle}:")
                            print(f"  Hypotenuse: {hypotenuse} = {hyp_val}")
                            print(f"  Adjacent: {adjacent} = {adj_val}")
                            print(f"  Opposite: {opposite} = {opp_val}")
                            print(f"  Calculated {func_name}({angle_token}) = {calculated_val}")
    ...
    def adding_conclution(self, theorem_name: str, args: List[str], premise: str, conclusions: List[str]) -> \
        ...
        # Add this to the cosine_theorem handler in your adding_conclution method
            ...
            if version == "1":
                ...
                # Create sine variable for this angle
                ...
                # Add the Pythagorean identity constraint: sin²(θ) + cos²(θ) = 1
                # But first check if this identity is already added to avoid redundancy
                identity_exists = False
                for constraint in self.solver.assertions():
                    if str(constraint) == str(sin_var * sin_var + cos_var * cos_var == 1):
                        identity_exists = True
                        break

                if not identity_exists:
                    self.solver.add(sin_var * sin_var + cos_var * cos_var == 1)
                    print(f"Added Pythagorean identity constraint: sin²({angle_str}) + cos²({angle_str}) = 1")

                # Handle right triangles specifically
                triangle_name = args[1]
                if self.normalize_triangle(triangle_name) in self.right_triangles:
                    print(f"Triangle {triangle_name} is a right triangle")

                    # Find the right angle vertex
                    right_angle_vertex = None
                    for v in triangle_name:
                        # Check angles involving this vertex
                        others = [p for p in triangle_name if p != v]
                        angle_name = others[0] + v + others[1]
                        normalized_angle = self.normalize_angle_name(angle_name)
                        angle_key = f"angle_{normalized_angle}"

                        if angle_key in self.angles:
                            # Test if this is the right angle
                            angle_var = self.angles[angle_key]
                            temp_solver = Solver()
                            for c in self.solver.assertions():
                                temp_solver.add(c)
                            temp_solver.add(angle_var != 90)
                            ...
                            if temp_solver.check() == unsat:
                                ...
                                print(f"Found right angle at vertex {v}")
                                break

                    if right_angle_vertex:
                        # The key is that we're dealing with angle BAC in triangle ABC where C is the right angle
                        vertex = angle_str[1]  # Middle letter is the vertex of our angle

                        if vertex != right_angle_vertex and self.solver.check() == sat:
                            # We have a non-right angle in a right triangle

                            # For a right triangle, we can determine the sides directly
                            # Hypotenuse: the side opposite to the right angle
                            hypotenuse_vertices = [v for v in triangle_name if v != right_angle_vertex]
                            hypotenuse = "".join(sorted(hypotenuse_vertices))

                            # Get the actual measurements from the model
                            model = self.solver.model()

                            # In a right triangle ACB with right angle at C:
                            # For angle BAC:
                            #   Opposite side: BC
                            #   Adjacent side: AC
                            #   Hypotenuse: AB

                            # Correctly identify the sides for our specific angle
                            opposite_side = None
                            adjacent_side = None

                            # Our angle is at A, vertex is A
                            # The right angle is at C
                            # The opposite side to A is BC - side opposite to vertex
                            # The adjacent side to A is AC - side connecting vertex to right angle

                            # Opposite side: side connecting the right angle to the other vertex
                            other_vertex = [v for v in triangle_name if v != vertex and v != right_angle_vertex][0]
                            opposite_side = "".join(sorted([right_angle_vertex, other_vertex]))

                            # Adjacent side: side connecting our angle's vertex to the right angle vertex
                            adjacent_side = "".join(sorted([vertex, right_angle_vertex]))

                            # Get the side lengths
                            hyp_var = self.add_length(hypotenuse[0], hypotenuse[1])
                            opp_var = self.add_length(opposite_side[0], opposite_side[1])
                            adj_var = self.add_length(adjacent_side[0], adjacent_side[1])

                            # Get the actual values from the solver
                            try:
                                hyp_val = float(model.eval(hyp_var).as_decimal(10).rstrip('?'))
                                opp_val = float(model.eval(opp_var).as_decimal(10).rstrip('?'))
                                adj_val = float(model.eval(adj_var).as_decimal(10).rstrip('?'))

                                # Calculate the exact trig values from these lengths
                                if hyp_val > 0:  # Avoid division by zero
                                    sin_val = opp_val / hyp_val
                                    cos_val = adj_val / hyp_val

                                    # Check if we already have constraints for these values
                                    sin_constraint_exists = False
                                    cos_constraint_exists = False

                                    for constraint in self.solver.assertions():
                                        if str(constraint) == str(sin_var == opp_var / hyp_var):
                                            sin_constraint_exists = True
                                        if str(constraint) == str(cos_var == adj_var / hyp_var):
                                            cos_constraint_exists = True

                                    # Only add constraints if they don't exist yet
                                    if not sin_constraint_exists:
                                        self.solver.add(sin_var == opp_var / hyp_var)
                                        print(
                                            f"Added right triangle constraint: sin({angle_str}) = |{opposite_side}|/|{hypotenuse}| = {sin_val}")

                                    if not cos_constraint_exists:
                                        self.solver.add(cos_var == adj_var / hyp_var)
                                        print(
                                            f"Added right triangle constraint: cos({angle_str}) = |{adjacent_side}|/|{hypotenuse}| = {cos_val}")

                                    # For our specific problem, we want to ensure the right values:
                                    if angle_str == "BAC" and right_angle_vertex == "C":
                                        print(f"For angle BAC in triangle with right angle at C:")
                                        print(f"  Hypotenuse: |AB| = {hyp_val}")
                                        print(f"  Opposite: |BC| = {opp_val}")
                                        print(f"  Adjacent: |AC| = {adj_val}")
                                        print(f"  sin(BAC) = |BC|/|AB| = {opp_val}/{hyp_val} = {sin_val}")
                            except Exception as e:
                                print(f"Error calculating trig values: {e}")
...
if __name__ == "__main__":
    result, feedback = verify_geometric_proof(
        "/Users/eitan/Desktop/lean/lean_python/questions/the new format for questions after jan_17/new_45_questions/question_4187/question4187_gt",
        print_output=False)
