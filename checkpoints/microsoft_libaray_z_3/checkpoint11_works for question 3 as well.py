...
class GeometricTheorem:
    ...
    def add_perpendicular_constraint(self, line1: str, line2: str):
        """Add 90° angle constraint between perpendicular lines"""
        angle_var = self.add_angle(line1[0], line1[1], line2[1])
        self.solver.add(angle_var == 90)
        self.known_angle_values[self.normalize_angle_name(line1[0] + line1[1] + line2[1])] = 90.0
    ...
    def are_equal_angles(self, angle1: str, angle2: str) -> bool:
        """Check if two angle names refer to the same angle"""
        if len(angle1) != 3 or len(angle2) != 3:
            return False

        # Either direct match or reversed match
        return angle1 == angle2 or (angle1[2] + angle1[1] + angle1[0] == angle2)
    ...
    def validate_theorem_premises(self, theorem_name: str, args: List[str], premise: str) -> Tuple[
        ...
        elif "PerpendicularBetweenLine" in premise:
            line_match = re.search(r'PerpendicularBetweenLine\((\w+),\s*(\w+)\)', premise)
            if line_match:
                line1, line2 = line_match.group(1), line_match.group(2)
                if (line1, line2) not in self.perpendicular_pairs and (line2, line1) not in self.perpendicular_pairs:
                    return return_error(GeometricError(
                        tier=ErrorTier.TIER2_PREMISE,
                        message=f"Lines {line1} and {line2} not proven perpendicular",
                        details=f"Available perpendicular pairs: {self.perpendicular_pairs}"
                    ))
            ...
            if 'TEXT_CDL' in sections:
                for line in sections['TEXT_CDL']:
                    if line.startswith('Equal(MeasureOfAngle('):
                        ...
                        if match:
                            line1, line2 = match.group(1), match.group(2)  # BC, AC
                            ...
                            # For BC ⊥ AC, we want BCA (between the lines)

                            angle1 = f"BCA"  # Between the two lines

                            angle2 = f"ACB"  # The reverse

                            print(f"Adding 90° angles for perpendicular intersection: {angle1} and {angle2}")

                            self.known_angle_values[angle1] = 90.0

                            self.known_angle_values[angle2] = 90.0
